...
from hello import locations_data
TOKEN = "6724060662:AAHOhLjRD9FWyUCUJrKOBKm8FviDf7kCif0"
bot = telebot.TeleBot(TOKEN)


def load_user_data():
    try:
        with open("death_notebook.json", "r") as file:
            return json.load(file)
    except:
        return {}

def save_user_data(user_data):
    with open("death_notebook.json", "w") as file:
        json.dump(user_data, file)

# Структура для хранения данных пользователя
if os.path.exists("death_notebook.json"):
...
def start_quest(message):
    global user_data
    if str(message.chat.id) in user_data and user_data[str(message.chat.id)]['location'] not in ['find_path', 'approach_kaif', 'explore_wasted', 'talk_to_google']:
...
def send_location(chat_id):
    ...
    picture_path = location_data["picture"] #  путь к картинке
